...
describe("Grok subscription usage", () => {
  it("maps the current weekly subscription pool", () => {
    expect(
      buildGrokSubscriptionUsageFromBilling({
  ...
  it.effect("uses Grok's billing ACP extension", () =>
    Effect.gen(function* () {
      const requests: Array<{ method: string; payload: unknown }> = [];
      const usage = yield* requestGrokSubscriptionUsage((method, payload) => {
        requests.push({ method, payload });
        return Effect.succeed({ config: null, subscription_tier: "SuperGrok" });
      });
      expect(requests).toEqual([{ method: "_x.ai/billing", payload: {} }]);
      expect(usage).toEqual({ provider: "grok", plan: "SuperGrok", windows: [] });
    }),
  );
});
